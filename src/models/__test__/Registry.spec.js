...
describe('Model Registry', () => {
	class MOCK { static MimeType = 'application/vnd.nextthought.foo'}
	class MOCK2 { static MimeType = 'application/vnd.nextthought.bar'}
	...
	describe('Static API', () => {
		test('register', () => {
			...
			expect(() => Registry.register({})).toThrow();
			expect(() => Registry.register({MimeType: 'application/vnd.nextthought.foo'})).toThrow();
			...
			spyOn(logger, 'warn');
			expect(Registry.register(MOCK)).toBe(MOCK);
			expect(logger.warn).toHaveBeenCalledWith('Overriding Type!! %s with %o was %o', 'foo', MOCK, MOCK);
		});
		...
		test('alias', () => {
			...
			expect(() => Registry.alias({}, {})).toThrow('aliases and types must be strings');
			expect(() => Registry.alias(1, 2)).toThrow('aliases and types must be strings');
			expect(() => Registry.alias('', {})).toThrow('aliases and types must be strings');
			expect(() => Registry.alias({}, '')).toThrow('aliases and types must be strings');

			spyOn(logger, 'warn');

			expect(() => Registry.alias('cap', 'baz')).toThrow('Cannot alias a non-existing type');
			expect(() => Registry.alias('foo', 'foo')).toThrow('Cannot create an alias that is self-referencing.');
			expect(() => Registry.alias('bar', 'foo')).toThrow('Cannot alias over an existing type.');
			expect(() => Registry.alias('foo', 'baz')).not.toThrow();
			expect(() => Registry.alias('foo', 'baz')).not.toThrow();
			expect(logger.warn).toHaveBeenCalledWith('Overriding Type!! %s with %o was %o', 'baz', 'foo', 'foo');
		});
		...
		test('lookup', () => {
			...
			expect(Registry.lookup('application/vnd.nextthought.foo')).toBe(MOCK);
	...
	describe ('Instance', () => {
		...
		test('lookup', () => {
			...
			expect(logger.debug).toHaveBeenCalledWith('Alias Loop Detected... already seen: %s\n\t=?> %s', 'zab\n\t=> baz\n\t=> zaz', 'zab');
		});
	});
});
