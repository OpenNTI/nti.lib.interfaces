		...
		const service = {
			getMetadataFor: () => {
				...
				return Promise.resolve({
					contentLocation: 'https://cdnapi.kaltura.com/p/' + PARTNER_ID + '/sp/55500/something/entry_id/' + ID
				});
			}
		};
	...
	test('Resolve ID, with images', async () => {
		...
		const service = {
			getMetadataFor: () => {
				// meta object
				return Promise.resolve({
					contentLocation: 'https://video.kaltura.com/media/Some+Title/' + ID,
					images: [
						{
							Class: 'ImageMetadata',
							MimeType: 'application/vnd.nextthought.metadata.imagemetadata',
							url: 'https://cdnapi.kaltura.com/p/' + PARTNER_ID + '/sp/55500/something/entry_id/' + ID
						},
						{
							Class: 'ImageMetadata',
							MimeType: 'application/vnd.nextthought.metadata.imagemetadata',
