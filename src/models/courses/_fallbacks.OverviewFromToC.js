...
const getEnrollmentStatus = outlineNode =>
	outlineNode.parent('isEnrollment').getStatus();
const isVisible = (outlineNode, overviewItem) =>
	outlineNode.hasVisibility(overviewItem, getEnrollmentStatus(outlineNode));
...
export default function buildFromToc(element, outlineNode) {
	...
	for (let node of element) {
		...
		let type =
			obj && (obj.section || SECTION_TYPE_MAP[obj.MimeType] || 'Unknown');

		if (!type || !isVisible(outlineNode, obj)) {
			continue;
		}
