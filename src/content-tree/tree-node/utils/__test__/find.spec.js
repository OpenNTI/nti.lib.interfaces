	...
	describe('function predicate', () => {
		const getChildren = () => [
			create('node-1', [
				create('node-1-1', [
					create('node-1-1-1'),
					create('node-1-1-2', [create('node-1-1-2-1')]),
				]),
			]),
			create('node-2'),
			create('node-3', [
				create('node-3-1'),
				create('node-3-2', [create('node-3-2-1')]),
			]),
		];
		...
		describe('non-recursive', () => {
			...
			test('finds first node that matches', async () => {
				...
				const found = await find(
					children,
					makeNamePredicate('node-3', 'node-2')
				);
		...
		describe('recursive', () => {
			...
			test('does breadth first search', async () => {
				...
				for (let check of toCheck) {
					...
					const found = await find(
						children,
						makeNamePredicate(second, first),
						true
					);
