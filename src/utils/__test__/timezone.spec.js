...
describe('TimeZone setup tools', () => {
	...
	test('getTimezoneFromCookie()', () => {
		global.document = {
			cookie:
				'timezone=' +
				encodeURIComponent('{"offset":1,"name":"FooBar"}'),
		};
	...
	test('setTimezoneCookie()', () => {
		...
		expect(global.document.cookie).toEqual(
			'timezone=%7B%22test%22%3A1%7D; path=/'
		);
	});
	...
	test('getTimezone()', () => {
		...
		expect(global.document.cookie).toEqual(
			'timezone=' + encodeURIComponent(JSON.stringify(o)) + '; path=/'
		);
		...
		expect(global.document.cookie).toEqual(
			'timezone=%7B%22offset%22%3A1%2C%22name%22%3A%22FooBar%22%7D; path=/'
		);
	});
});
