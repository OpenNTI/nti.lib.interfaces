...
const getInstances = service =>
	service.getDataCache().get('LibraryInstances', {}, true);
...
export default class Library extends EventEmitter {
	static get(service, name, reload) {
		...
		return (instances[name] = new Library(service, name));
	}
	...
	async load() {
		...
		try {
			await Promise.all([
				loadCollection(
					this,
					this.service.getContentBundlesURL(),
					'bundles',
					filterBadBundles
				),
				loadCollection(
					this,
					this.service.getCoursesEnrolledURL(),
					'courses'
				),
				loadCollection(
					this,
					this.service.getCoursesAdministeringURL(),
					'administeredCourses'
				),
			]);
		} finally {
